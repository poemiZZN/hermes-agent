    ...
    for msg in messages:
        ...
        if role == "assistant" and msg.get("tool_calls"):
            ...
            if len(kept_tcs) != len(msg.get("tool_calls") or []):
                # If every call was a duplicate, omitting the field is the
                # only provider-safe representation. Re-introducing
                # ``tool_calls: []`` here undoes the normalization pass above
                # and strict OpenAI-compatible providers reject the request.
                if kept_tcs:
