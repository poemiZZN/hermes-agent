...
只有在用户明确描述，或当前会话确实能看见参考资产内容时，才能为 `<Subject N>` 或 `<Picture N>` 写入角色、场景、服装、道具、构图等视觉特征。看不到或无法确认时，只写该资产的功能和来源，例如 `<Picture 1> is the first-frame reference for [Shot 1].`；不要根据文件名、上下文或常识猜测画面内容。
...
3. 用固定英文保留字段依次输出：`subject_definitions`、`summary`、`retention_analysis`、`detailed_description`、`overall_soundscape`、`non_diegetic_music`。字段中的描述性正文跟随用户源文本的语言；用户输入混合语言或无法判断时，使用中文。
...
- 保持所有手册保留字为英文：六个章节名、`<Subject N>`/`<Picture N>`/`<Video N>`/`<Audio N>`、任务类型、保留关系标记、`[Shot N]`、`At MM:SS.mmm`、`(Sx)`、`<d>`、`[Language]`、`N/A`。
- 除保留字外，`subject_definitions`、`summary`、`retention_analysis`、`detailed_description`、`overall_soundscape` 和 `non_diegetic_music` 中的描述性文本跟随用户源文本语言。对白、歌词和可见文本始终保留其原始语言。
- 不要引入未定义的参考标签，不要改变同一标签的含义，不要在 `retention_analysis` 写 `(Sx)`。
- 对无法直接确认内容的参考资产，不要补写任何视觉特征或把它们错误地绑定到角色、场景或道具。
